...
export default function PresentationPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between p-6 lg:p-12 transition-colors select-none">
      {/* Header Controls Bar */}
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground tracking-tight">NovaRetail GCC — Boardroom Presentation</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-muted p-1 rounded-lg border border-border">
            <button onClick={prevSlide} className="p-1.5 hover:bg-card rounded-md text-foreground transition-colors" title="Previous slide (Left Arrow)">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={nextSlide} className="p-1.5 hover:bg-card rounded-md text-foreground transition-colors" title="Next slide (Right Arrow / Space)">
      ...
      <div className="my-8 flex-1 flex items-center justify-center">
          ...
          <div className="w-full max-w-4xl space-y-8 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="glass-panel p-6 rounded-2xl border border-border text-center space-y-2">
                ...
                <p className="text-3xl font-extrabold text-primary">{formatAED(metrics.totalInitialOutlay)}</p>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-border text-center space-y-2">
                ...
                <p className="text-3xl font-extrabold text-success">{formatAED(metrics.npv)}</p>
              ...
              <div className="glass-panel p-6 rounded-2xl border border-border text-center space-y-2">
                ...
                <p className="text-3xl font-extrabold text-purple-400">{formatPercent(metrics.irr || 0)}</p>
                <span className="text-[10px] text-muted-foreground font-mono">Hurdle: 11.5%</span>
              </div>
            </div>

            <div className={`p-6 rounded-2xl border flex items-center justify-between ${getDecisionBadgeColor(metrics.decisionStatus)}`}>
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <h3 className="text-2xl font-bold">{metrics.decisionStatus}</h3>
                </div>
              </div>
        ...
        {currentSlide === 1 && (
          <div className="w-full max-w-5xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
            <h2 className="text-2xl font-bold text-foreground flex items-center gap-2 border-b border-border pb-3">
              <TrendingUp className="h-6 w-6 text-primary" /> 6-Year Free Cash Flow Schedule (AED)
            </h2>

            <div className="glass-panel p-6 rounded-2xl border border-border overflow-x-auto">
              <table className="w-full text-left text-xs font-mono border-collapse">
                ...
                <tbody className="divide-y divide-border text-foreground">
                  ...
                  <tr className="hover:bg-muted/50">
                    ...
                    {cashFlows.map((cf) => (
                      <td key={cf.year} className="py-3 px-4 text-right text-amber-500">({formatAED(cf.additionalOpEx)})</td>
                    ))}
                  </tr>
                  <tr className="hover:bg-muted/50">
                  ...
                  <tr className="hover:bg-muted/50 font-extrabold bg-primary/10">
          ...
          <div className="w-full max-w-4xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="glass-panel p-4 rounded-xl border border-border text-center">
                ...
                <p className="text-xl font-bold text-amber-500 mt-1">{formatAED(metrics.npv * 0.45)}</p>
              </div>
        ...
        {currentSlide === 3 && (
          <div className="w-full max-w-4xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-border space-y-3 font-mono text-xs">
        ...
        {currentSlide === 4 && (
          <div className="w-full max-w-4xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-border space-y-3 text-xs font-sans">
      ...
      <div className="flex items-center justify-between border-t border-border pt-4 text-xs text-muted-foreground font-mono">
        <span>Use Left/Right arrow keys or Spacebar to navigate slides</span>
        <span>CapExIQ v1.0 • NovaRetail GCC Presentation Mode</span>
      </div>
    </div>
  );
}
