...
import { isMfaEnabled } from '@/lib/db/repositories/mfa';
...
export async function POST(req: Request) {
  ...
  if (isMfaEnabled(user.id)) {
    // No session yet. The challenge proves the password step succeeded and is
    // rejected by `verifySession`, so it cannot be presented as a cookie to
    // reach anything — the account stays unreachable until a code is checked.
    ...
    return NextResponse.json({
      mfaRequired: true,
      challenge,
      // Named so the form can say which app to open, without revealing
      // anything a caller who already has the password does not know.
      account: user.email,
    });
  }
