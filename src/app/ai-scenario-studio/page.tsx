    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-background text-sm px-4 py-2.5 rounded-card border border-border focus:outline-none focus:ring-1 focus:ring-primary text-foreground"
              />
              <button
                type="submit"
                disabled={loading || !promptText.trim()}
                className="px-5 py-2.5 bg-primary text-primary-foreground font-bold text-xs rounded-card hover:opacity-90 transition-opacity flex items-center gap-2 shrink-0 disabled:opacity-50"
              >
                {loading ? (
                  <>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border/60">
          ...
          {[
            ...
            <button
              ...
              className="text-[11px] px-2.5 py-1 rounded-card bg-muted hover:bg-card border border-border text-foreground transition-colors"
            >
              {preset}
            </button>
          ))}
      ...
      {generatedScenario && (
        <div className="space-y-6 animate-fadeIn">
          <div className="bg-gradient-to-br from-card via-card to-primary/5 border border-primary/20 rounded-card p-6 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
              <div>
                <span className="text-xs uppercase font-bold text-primary tracking-wider">Synthesized AI Scenario</span>
                <h3 className="text-xl font-semibold text-foreground mt-0.5">{generatedScenario.scenarioName}</h3>
              </div>
              <button
                onClick={applyCustomScenario}
                disabled={applied}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-card bg-success hover:opacity-90 text-success-foreground font-bold text-xs transition-colors shrink-0 disabled:opacity-50"
              >
                {applied ? (
                  <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
              <div className="bg-background/60 p-3 rounded-card border border-border text-center">
              ...
              <div className="bg-background/60 p-3 rounded-card border border-border text-center">
                ...
                <p className="text-lg font-bold text-success mt-0.5">{generatedScenario.multipliers.operatingBenefitMultiplier.toFixed(2)}x</p>
              </div>
              <div className="bg-background/60 p-3 rounded-card border border-border text-center">
                <span className="text-[10px] text-muted-foreground uppercase font-semibold">Operating Cost Multiplier</span>
                <p className="text-lg font-bold text-destructive mt-0.5">{generatedScenario.multipliers.operatingCostMultiplier.toFixed(2)}x</p>
              </div>
              <div className="bg-background/60 p-3 rounded-card border border-border text-center">
            ...
            <div className="pt-4 border-t border-border">
              <h4 className="text-xs font-bold text-foreground uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <Activity className="h-4 w-4 text-info" /> Fitted Monte Carlo Triangular Distribution
              </h4>
              <div className="flex items-center justify-around bg-background/50 p-3 rounded-card border border-border text-xs font-mono">
                ...
                <div>Maximum: <strong className="text-success">{generatedScenario.triangularDistribution.maxBenefitMultiplier.toFixed(2)}x</strong></div>
              </div>
            </div>
          </div>
        </div>
