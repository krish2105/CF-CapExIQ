  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border">
        <div>
          <h1 className="font-display text-[clamp(24px,2.6vw,32px)] leading-tight font-normal text-foreground flex items-center gap-2">
            <Box className="h-6 w-6 text-primary" /> 3D Spatial Digital Twin & Warehouse Layout Optimizer
          </h1>
        ...
        <button
          ...
          disabled={simulating}
          className="px-4 py-2 rounded-card bg-primary hover:opacity-90 text-primary-foreground font-bold text-xs flex items-center gap-2 shrink-0 transition-colors"
        >
          {simulating ? (
            <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 bg-card border border-border rounded-card p-6 space-y-4 flex flex-col justify-between min-h-[420px]">
          <div className="flex items-center justify-between border-b border-border pb-3">
            <div className="flex items-center gap-2 text-xs font-bold text-foreground">
              <Layers className="h-4 w-4 text-primary" /> Interactive Spatial Grid — Dubai South MFC (3,500 sq meters)
            </div>
            <div className="flex gap-1.5 font-mono text-xs">
              {(['A', 'B', 'C'] as const).map((zone) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-card font-bold transition-all ${
                    activeZone === zone
          ...
          <div className="relative flex-1 bg-surface border border-border rounded-card p-6 flex flex-col justify-between overflow-hidden">
            {/* Grid Lines */}
            ...
            <div className="relative z-10 grid grid-cols-4 sm:grid-cols-6 gap-3">
              {[...Array(12)].map((_, i) => (
                ...
                >
                  ...
                  <span className="text-[9px] text-muted-foreground">Cap: 94%</span>
                </div>
              ))}
            </div>

            {/* AMR Fleet Activity Visualizer */}
            <div className="relative z-10 pt-6 border-t border-border flex items-center justify-between text-xs font-mono text-card-foreground">
              <div className="flex items-center gap-2">
                <Cpu className="h-4 w-4 text-primary animate-pulse" />
                <span>Active AMRs: <strong className="text-foreground">{robotCount} Units</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <Zap className="h-4 w-4 text-warning" />
                <span>Throughput: <strong className="text-foreground">8,450 Orders/Day</strong></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-card border border-border rounded-card p-6 space-y-5">
          <h3 className="text-sm font-bold text-foreground border-b border-border pb-3 flex items-center gap-2">
            <Activity className="h-4 w-4 text-primary" /> Spatial AI Layout Optimizer
          </h3>
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="bg-background/60 p-3 rounded-card border border-border space-y-2">
              <div className="flex justify-between font-semibold">
                <span className="text-muted-foreground">Order Pick SLA Time</span>
                <span className="text-success font-mono">1.8 Mins / Tote</span>
              </div>
              <div className="flex justify-between font-semibold">
                <span className="text-muted-foreground">Space Utilization</span>
                <span className="text-primary font-mono">92.4% Optimal</span>
              </div>
              <div className="flex justify-between font-semibold">
                <span className="text-muted-foreground">Congestion Bottleneck</span>
                <span className="text-success font-mono">0.2% Low</span>
              </div>
            </div>

            <div className="bg-primary/5 border border-primary/20 rounded-card p-3 space-y-1">
