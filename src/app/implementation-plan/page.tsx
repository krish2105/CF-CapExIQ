        ...
        <table className="ledger-table">
          <thead>
            <tr>
              ...
              <th className="py-2.5 px-3 num">Budget (AED)</th>
          ...
          <tbody>
            {DEFAULT_STAGE_GATES.map((gate) => (
              <tr key={gate.id} className="hover:bg-muted/50">
                ...
                <td className="py-2.5 px-3 text-center font-bold text-info">{gate.plannedMonths}</td>
                <td className="py-2.5 px-3 num font-bold">{formatAED(gate.budgetAllocatedAed)}</td>
                ...
                <td className="py-2.5 px-3 text-center font-sans font-bold">
                  <span className={`px-2 py-0.5 rounded text-[10px] ${
                    gate.gateDecision === 'Continue' ? 'bg-success/20 text-success' : gate.gateDecision === 'Continue with Conditions' ? 'bg-warning/20 text-warning' : 'bg-muted text-muted-foreground'
                  }`}>
                    {gate.gateDecision}
                  </span>
                </td>
