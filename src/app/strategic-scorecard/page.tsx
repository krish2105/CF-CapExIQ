  ...
  return (
    <div className="space-y-6">
      ...
      {scorecardResult.tradeoffNotice && (
        <div className="p-4 rounded-card bg-warning/10 border border-warning/30 text-warning text-xs space-y-1">
          <div className="flex items-center gap-2 font-bold">
            <AlertTriangle className="h-4 w-4 text-warning" /> Strategic Trade-off Disclosure:
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className={`p-4 rounded-card border flex flex-col justify-between ${
              npv > 0 && scorecardResult.weightedScore >= 3.5 ? 'bg-success/15 border-success text-success font-bold' : 'bg-muted/40 border-border text-muted-foreground'
            }`}>
              ...
              {npv > 0 && scorecardResult.weightedScore >= 3.5 && (
                <span className="mt-2 text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-success text-success-foreground w-fit">Active Position</span>
              )}
            </div>

            <div className={`p-4 rounded-card border flex flex-col justify-between ${
              npv <= 0 && scorecardResult.weightedScore >= 3.5 ? 'bg-warning/15 border-warning text-warning font-bold' : 'bg-muted/40 border-border text-muted-foreground'
            }`}>
            ...
            <div className={`p-4 rounded-card border flex flex-col justify-between ${
              npv > 0 && scorecardResult.weightedScore < 3.5 ? 'bg-primary/15 border-primary text-primary font-bold' : 'bg-muted/40 border-border text-muted-foreground'
            }`}>
            ...
            <div className={`p-4 rounded-card border flex flex-col justify-between ${
              npv <= 0 && scorecardResult.weightedScore < 3.5 ? 'bg-destructive/15 border-destructive text-destructive font-bold' : 'bg-muted/40 border-border text-muted-foreground'
            }`}>
        ...
        <table className="ledger-table">
          <thead>
            <tr>
              ...
              <th className="py-2.5 px-3 num">Weighted Points</th>
            </tr>
          </thead>
          <tbody>
            {dimensions.map((d) => (
              <tr key={d.id} className="hover:bg-muted/50">
                ...
                <td className="py-2.5 px-3 num font-bold text-success">
